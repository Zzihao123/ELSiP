...
def train(datasets, cur, args):
    ...
    if args.model_type in ['clam_sb', 'clam_mb', 'our_clam_sb']:
        if args.subtyping:
        ...
        if args.model_type =='clam_sb':
            ...
            model = ELSiP(**model_dict, instance_loss_fn=instance_loss_fn)
        else:
    ...
    for epoch in range(args.max_epochs):
        if args.model_type in ['clam_sb', 'clam_mb'] and not args.no_inst_cluster:     
            ...
            train_loop_clam_our(epoch, model, train_loader, optimizer, args.n_classes, args.bag_weight, writer, loss_fn, args.enhance)
            stop = validate_clam_our(cur, epoch, model, val_loader, args.n_classes, 
                early_stopping, writer, loss_fn, args.results_dir, args.enhance)
    ...
    if args.model_type == 'our_clam_sb':
        _, val_error, val_auc, _= summary_our(model, val_loader, args.n_classes, args.enhance)
    else:
        _, val_error, val_auc, _= summary(model, val_loader, args.n_classes)
    print('Val error: {:.4f}, ROC AUC: {:.4f}'.format(val_error, val_auc))

    if args.model_type == 'our_clam_sb':
        results_dict, test_error, test_auc, acc_logger = summary_our(model, test_loader, args.n_classes, args.enhance)
    else:
...
def train_loop(epoch, model, loader, optimizer, n_classes, writer = None, loss_fn = None):   
    ...
    for batch_idx, batch_data in enumerate(loader):
        ...
        train_loss += loss_value
        if (batch_idx + 1) % 200 == 0:
            print('batch {}, loss: {:.4f}, label: {}, bag_size: {}'.format(batch_idx, loss_value, label.item(), patch_features.size(0)))
...
def train_loop_clam_our(epoch, model, loader, optimizer, n_classes, bag_weight, writer = None, loss_fn = None, enhance=False):   
    model.train()
    acc_logger = Accuracy_Logger(n_classes=n_classes)
    inst_logger = Accuracy_Logger(n_classes=n_classes)
    ...
    for batch_idx, batch_data in enumerate(loader):
        ...
        if (batch_idx + 1) % 200 == 0:
            print('batch {}, loss: {:.4f}, instance_loss: {:.4f}, weighted_loss: {:.4f}, '.format(batch_idx, loss_value, instance_loss_value, total_loss.item()) + 
                'label: {}, bag_size: {}'.format(label.item(), patch_features.size(0)))
...
def validate_clam_our(cur, epoch, model, loader, n_classes, early_stopping = None, writer = None, loss_fn = None, results_dir = None, enhance=False):
    model.eval()
    acc_logger = Accuracy_Logger(n_classes=n_classes)
    inst_logger = Accuracy_Logger(n_classes=n_classes)
    val_loss = 0.
...
def summary_our(model, loader, n_classes, enhance=False):
    acc_logger = Accuracy_Logger(n_classes=n_classes)
    model.eval()
    test_loss = 0.
    test_error = 0.
