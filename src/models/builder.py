...
def get_encoder(model_name, target_img_size=224):
    ...
    if model_name == 'resnet50_trunc':
        # 检查是否有本地ResNet权重文件
        local_resnet_path = "/data/zzh/WSI_zzh/CLAM/pytorch_model.bin"
        if os.path.exists(local_resnet_path):
            print(f"Using local ResNet weights: {local_resnet_path}")
            model = TimmCNNEncoder(local_weights_path=local_resnet_path)
        else:
            print("No local ResNet weights found, using pretrained model")
        ...
        assert target_img_size == 512, 'TITAN is used with 448x448 CONCH v1.5 features'
    else:
